...
- [x] データベーススキーマが作成され、マイグレーション可能
- [x] 判例データの取得・保存が動作する（プレースホルダー実装、実際のスクレイピングは裁判所サイト調査後）
- [x] 全文検索が動作する（PostgreSQL GINインデックス + to_tsvector実装）
- [x] 全APIエンドポイントが実装され動作する
- [x] 単体テストカバレッジ80%以上
- [x] 統合テストが全てパス
- [x] バッチ処理が正常に動作する
- [x] Swagger UIで仕様が確認できる
